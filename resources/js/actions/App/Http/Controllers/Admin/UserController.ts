 ...
 */
export const updateBuyerAccess = (
    args:
        | { user: number | { id: number } }
        | [user: number | { id: number }]
        | number
...
updateBuyerAccess.url = (
    ...
    return (
        updateBuyerAccess.definition.url
            .replace('{user}', parsedArgs.user.toString())
            .replace(/\/+$/, '') + queryParams(options)
    );
};
